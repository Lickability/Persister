...
public final class LRUCache<Key: Hashable & Sendable, Value: Sendable>: Sendable {
    ...
    nonisolated(unsafe) private var keysOrderedByRecentUse = SynchronizedArray<Key>()
    nonisolated(unsafe) private var backingStoreDictionary = SynchronizedDictionary<Key, Value>()
    nonisolated(unsafe) private var expirationDictionary = SynchronizedDictionary<Key, Date>()
